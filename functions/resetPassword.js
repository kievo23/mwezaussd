...
let resetPassword = (customer,text,req,res) => {
    ...
    console.log(size);
    if(size == 1 && text == ""){
        let response =`CON Welcome to ${config.app.name} Platform
        ...
        if(newPasword == confirmPassword){
            ...
            customer.save((err, success) => {
                if(err){
                    ...
                    let response =`END Congratulations, your account is now active. You can now access ${config.app.name} services`
                    res.send(response)
                }
            })
            let response =`END Congratulations, your account is now active. You can now access ${config.app.name} services`
